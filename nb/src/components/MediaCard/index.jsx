...
function Preview({ media }) {
    ...
    return (
        <div className="nb-media__stage">
            <img className="nb-media__img" src={media.originalUrl} alt="Sticker preview" loading="lazy" />
        </div>
    );
}
...
export function MediaCard({
    ...
    return (
        <article className={`nb-media ${active ? 'nb-media--active' : ''}`} data-file-id={media.fileId}>
            ...
            <div className="nb-media__actions">
                <Button size="sm" icon="download" onClick={onDownloadOriginal}>
                    原文件
                </Button>
                {media.type === 'webp' && (
                    <Button size="sm" variant="outline" icon="transform" loading={converting} onClick={onConvertPng}>
                        PNG
                    </Button>
                )}
                <Button size="sm" variant="outline" icon="copy" onClick={onCopyLink}>
                    链接
                </Button>
                {media.setName && (
                    <Button size="sm" variant="accent" icon={setButtonIcon} loading={setBusy} onClick={onToggleSet}>
                        {setButtonLabel}
                    </Button>
                )}
            </div>
